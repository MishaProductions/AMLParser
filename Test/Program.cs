...
public unsafe struct AcpiHeader
{
    /// <summary>
    /// Signature.
    /// </summary>
...
namespace Cosmoss.Core
{
    public unsafe partial class ACPI
    {
        ...
        {
            lai_warn("No implemenation: DoRevMethod");
        }

        private static void DoOSIMethod()
        {
            lai_warn("No implemenation: DoOSIMethod");
        }
        /// <summary>
        ...
        public static void lai_create_namespace()
        {
            ...
            //}
            fixed (byte* ptr = readtbl())
            {
                var tb = lai_load_table(ptr, 0);
        ...
        static void ReadHeader(BinaryReader _reader)
        {
            if (_reader == null)
                throw new("reader should not be null");
            Console.WriteLine("SDT header:");
            ...
            //Length
            uint _sdtLength = _reader.ReadUInt32();
            Console.WriteLine("\tLendth: " + _sdtLength.ToString() + " / " + _sdtLength.ToString("X2"));
        ...
        static byte[] tabl;
        private static byte[] readtbl()
        {
            if (tabl != null)
                return tabl;

            var _reader = new BinaryReader(new MemoryStream(File.ReadAllBytes(@"D:\1Misha\Downloads\iasl-win-20220331 (1)\application.aml")));
            ...
            tabl = ReadToEnd(_reader.BaseStream);
            return tabl;
        }
      ...
      || character == MULTI_PREFIX || character == DUAL_PREFIX)
        ...
        private static int lai_exec_parse(int parse_mode, ref lai_state state)
        {
            ...
            if (lai_is_name(method[pc]))
            {
                ...
                {
                    lai_panic("WARMING: RESOLVING NOT IMPLEMETNATION!");
            ...
            switch (opcode)
            {
                ...
                case 0xA0:
                    {
                        ...
                        if (pc < block.limit && method[pc] == 0xA1)
                        {
                            has_else = 1;
                            pc++;
                        ...
                        lai_exec_commit_pc(ref state, pc);
                        lai_exec_push_blkstack(ref state, if_pc, opcode_pc + 1 + if_size);
                        ...
                        state.stack_base[state.stack_base.Count - 1] = cond_item;
                        break;
                    }
                case 0xA1:
                    {
              ...
              || lai_parse_name(ref region_amln, method, ref pc, limit))
                            return 5;
        ...
        private static bool lai_parse_u16(ref ushort outt, byte[] method, ref int pc, int limit)
        {
            if (pc + 2 > limit)
                return true;
            outt = (ushort)(((ushort)method[pc]) | (((ushort)method[pc + 1]) << 8));
            pc += 2;
            return false;
        }
        ...
        private static bool lai_parse_u32(ref uint output, byte[] code, ref int pc, int limit)
        {
            ...
            output = ((uint)code[pc]) | (((uint)code[pc + 1]) << 8) | (((uint)code[pc + 2]) << 16) | (((uint)code[pc + 3]) << 24); ;
            pc += 4;
            return false;
        }
        ...
        {
            //Kernel.PrintDebug("LAI: ERROR: " + error);
        ...
        private static void lai_warn(string warn)
        {
            ...
            //Kernel.PrintDebug("LAI: WARN: " + warn);
        }
        ...
        private static void lai_install_nsnode(lai_nsnode node)
        {
            if (node.parent != null)
            {
                ...
                lai_log("installing node: " + node.name);
